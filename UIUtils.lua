...
function BoxxyAuras.UIUtils.QueueTooltipScrape(spellId, targetAuraInstanceID, filter, retryCount)
    ...
    if BoxxyAuras.AllAuras[targetAuraInstanceID] and not BoxxyAuras.AllAuras[targetAuraInstanceID].isPlaceholder then
        if BoxxyAuras.DEBUG then
            print(string.format("QueueTooltipScrape: Already have cached data for instanceID=%s, skipping queue",
                tostring(targetAuraInstanceID)))
        end
        return
    end
    ...
    for _, queuedItem in ipairs(BoxxyAuras.scrapingQueue) do
        if queuedItem.targetAuraInstanceID == targetAuraInstanceID then
            if BoxxyAuras.DEBUG then
                print(string.format("QueueTooltipScrape: instanceID=%s already queued, skipping",
                    tostring(targetAuraInstanceID)))
            end
            return
        end
...
function BoxxyAuras.UIUtils.DrawSlicedBG(frame, textureKey, layer, shrink)
    ...
    elseif layer == "hoverBorder" then
        if not frame.hoverBorderTextures then
            frame.hoverBorderTextures = {};
        end
        group = frame.hoverBorderTextures;
        subLevel = -6; -- Above normal border
    else
        return
    end
    ...
    -- Approach #1: Apply PixelUtil alignment to parent frame before drawing sliced backgrounds
    if PixelUtil and PixelUtil.SetPoint then
        local point, relativeTo, relativePoint, xOfs, yOfs = frame:GetPoint()
        if point and relativeTo and relativePoint then
            local success, err = pcall(PixelUtil.SetPoint, frame, point, relativeTo, relativePoint, xOfs, yOfs)
            if not success and BoxxyAuras.DEBUG then
                print(string.format("|cffFFFF00DrawSlicedBG Warning:|r Failed to pixel-align parent frame: %s",
                    tostring(err)))
            end
        end
    end
    ...
    for i = 1, 9 do
        ...
        -- Apply texel snapping bias for crisp texture rendering (optimized)
        if tex.SetTexelSnappingBias then
            tex:SetTexelSnappingBias(0.0)
        end

        -- Set texture (optimized - removed pcall for better performance)
...
function BoxxyAuras.UIUtils.ColorBGSlicedFrame(frame, layer, r, g, b, a)
    ...
    if group then
        -- Batch process textures for better performance
        -- Remove pcall overhead for better performance - SetVertexColor rarely fails
...
function BoxxyAuras.UIUtils.TryGameTooltipAuraScrape(spellId, targetAuraInstanceID, filter, retryCount)
    ...
    C_Timer.After(checkDelay, function()
        ...
        C_Timer.After(retryDelay, function()
            ...
            if BoxxyAuras.DEBUG then
                print(string.format(
                    "TryGameTooltipAuraScrape: Failed on attempt %d, completing scrape (avoiding spell ID fallback for aura)",
                    retryCount + 1))
            end
...
function BoxxyAuras.UIUtils.TryGameTooltipSpellScrape(spellId, targetAuraInstanceID, retryCount)
    ...
    C_Timer.After(checkDelay, function()
        ...
        C_Timer.After(retryDelay, function()
            ...
            if BoxxyAuras.DEBUG then
                print(string.format("TryGameTooltipSpellScrape: Failed on attempt %d, trying basic spell info",
                    retryCount + 1))
            end
    ...
    if not spellName then
        ...
        C_Timer.After(0.1, function()
            ...
            if spellName and spellName ~= "" then
                ...
                if BoxxyAuras.DEBUG then
                    print(string.format(
                        "TryBasicSpellInfoScrape: Success via tooltip, using spell name '%s' for instanceID=%s",
                        spellName, tostring(targetAuraInstanceID)))
                end
            else
                if BoxxyAuras.DEBUG then
                    print(string.format(
                        "TryBasicSpellInfoScrape: Failed via tooltip, no spell name found for spellId=%s",
                        tostring(spellId)))
                end
            end
...
function BoxxyAuras.UIUtils.ExtractTooltipLines(targetAuraInstanceID, source)
    ...
    if BoxxyAuras.DEBUG then
        print(string.format("ExtractTooltipLines (%s): Checking tooltip, spellName='%s'", source,
            tostring(spellNameFromTip)))
...
function BoxxyAuras.UIUtils.ScrapeAndCacheFromDummySpell(spellId, targetAuraInstanceID)
    ...
    if spellNameFromTip and #tooltipLines > 0 then
        ...
        if BoxxyAuras.DEBUG then
            print(string.format("ScrapeAndCacheFromDummySpell: Fallback scrape successful, cached %d lines",
                #tooltipLines))
        end
    else
        if BoxxyAuras.DEBUG then
